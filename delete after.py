...
class Creature(pygame.sprite.Sprite):
    ...
    def move(self, **directions):
        ...
        elif (xNew, yNew) in occupiedSpaces():
            positions = [(self.rect.x+1, self.rect.y+1), (self.rect.x+1, self.rect.y-1), (self.rect.x-1, self.rect.y+1), (self.rect.x-1, self.rect.y-1)]
            unoccupiedPoss = []
            for position in positions:
                if (position not in occupiedSpaces()) and (position[0] in range(0, drawSurfaceWidth)) and (position[1] in range(0, drawSurfaceWidth)):
                    unoccupiedPoss.append(position)
            if len(unoccupiedPoss) > 0:
                chance = random.randrange(101)
                if chance > 89:
                    addCreature(1, random.choice(unoccupiedPoss))
